...
映射是双射的，三角形不能翻转（flip over free）,三角形边不能自相交（self-intersection free），并且希望映射所造成的图像扭曲变形能尽可能小
...
上面约束优化问题虽然可以通过Lagrange乘子法转化为无约束优化问题进行求解，但是由于约束(3)中含有非线性条件所以是不容易求解的，一种思路如LinABF算法就是将约束(3)进行线性化后进行求解
